...
class Blockchain:
    ...
    def get_balance(self):
        tx_sender = [[tx.amount for tx in block.transactions if tx.sender == self.host]
                     for block in self.__chain]

        open_tx_sender = [
            tx.amount for tx in self.__open_transactions if tx.sender == self.host] 
        tx_sender.append(open_tx_sender)
        tx_receiver = [[tx.amount for tx in block.transactions if tx.receiver == self.host]
                       for block in self.__chain]
